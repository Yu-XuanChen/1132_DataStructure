...
private:
    ...
    TreeNode* deleteNode(TreeNode* node, int val) {
        if (val < node->data) { //如果要刪的值比目前這個節點小,往左子樹找
            ...
            if (!node->left && !node->right) { //節點沒有左子樹和右子數
                delete node; //刪除節點
                return nullptr; //回傳空指標
            }

            else if (!node->left) { //節點只有右子樹的話
                TreeNode* temp = node->right; //暫存右子樹
				delete node; //刪除節點
				return temp; //回傳右子樹
            }

			else if (!node->right) { //節點只有左子樹
                ...
                node->data = temp->data; //最小值放到目前的節點
				node->right = deleteNode(node->right, temp->data); //刪除右子樹的最小值
            }
        }
		return node; // 回傳更新後的節點
    }
